...
function getTotalCost(pricePerRefill, refills) {
  if(typeof pricePerRefill !== 'number' || typeof refills !== 'number') return 0;
  if(pricePerRefill <= 0 || refills <= 0) return 0;
...
function calculateCost() {
  ...
  const initialCost = getTotalCost(parseInt(pricePerRefill.value), parseInt(refills.value));
